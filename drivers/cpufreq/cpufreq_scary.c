							...
							cputime64_t *wall)
{
	...
	busy_time = cputime64_add(kstat_cpu(cpu).cpustat[CPUTIME_USER],
			kstat_cpu(cpu).cpustat[CPUTIME_SYSTEM]);

	busy_time = cputime64_add(busy_time, kstat_cpu(cpu).cpustat[CPUTIME_IRQ]);
	busy_time = cputime64_add(busy_time, kstat_cpu(cpu).cpustat[CPUTIME_SOFTIRQ]);
	busy_time = cputime64_add(busy_time, kstat_cpu(cpu).cpustat[CPUTIME_STEAL]);
	busy_time = cputime64_add(busy_time, kstat_cpu(cpu).cpustat[CPUTIME_NICE]);
		...
		const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		...
		if (dbs_tuners_ins.ignore_nice)
			dbs_info->prev_cpu_nice = kstat_cpu(j).cpustat[CPUTIME_NICE];
	}
	mutex_unlock(&dbs_mutex);
...
//    unsigned int new_freq;
	...
	for_each_cpu(j, policy->cpus) {
		...
		if (dbs_tuners_ins.ignore_nice) {
			...
			cur_nice = cputime64_sub(kstat_cpu(j).cpustat[CPUTIME_NICE],
			...
			j_dbs_info->prev_cpu_nice = kstat_cpu(j).cpustat[CPUTIME_NICE];
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			if (dbs_tuners_ins.ignore_nice) {
				j_dbs_info->prev_cpu_nice =
						kstat_cpu(j).cpustat[CPUTIME_NICE];
			}
		}
		this_dbs_info->down_skip = 0;
